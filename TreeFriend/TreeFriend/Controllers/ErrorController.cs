...
namespace TreeFriend.Controllers
{
    public class ErrorController : Controller
    {
        ...
        {
            var result = _db.Lectures.Where(x => x.Status == Models.Enum.LectureStatus.Launched && x.Count > 0 && x.EventDate >= DateTime.Now).OrderBy(x => x.LectureId).Take(4).Select(x => new LecturelistViewModel
            {
